...
const EventsDashboard = () => {
  ...
  const handleToggleRegistration = async (eventId) => {
    ...
    // Only change registration status if the event is still open
    if (eventToUpdate.isRegistrationOpen) {
      const updatedEvent = { ...eventToUpdate, isRegistrationOpen: false };

      try {
        const response = await fetch(
          `http://localhost:8080/api/events/${updatedEvent.id}`,
          {
            method: "PUT",
            headers: {
